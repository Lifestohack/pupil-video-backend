    ...
    def _run(self):
        while not self._stop:
            if self.newpayload == True:
                ...
                self.payload.setPayloadParam(self.backend.get_synced_pupil_time(self.monotonic), frame, self.frame_index)
        ...
        try:
            for f in stream:
                if backend.is_publishable():
                    ...
                    streamimage.dataready(frame[:,:,0], frame_index, capture_time)    #   give it to StartThreadToStream to publish
                    ''' 
